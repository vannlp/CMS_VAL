...
class Helpers
{
  ...
  public static function arrayToObject($array) {
    if (is_array($array)) {
        // Kiểm tra xem có key nào là chuỗi hay không
        $hasStringKeys = false;
        ...
        if ($hasStringKeys) {
            // Nếu có key là chuỗi, chuyển mảng thành object
            $result = new Collection();
            foreach ($array as $key => $value) {
                // Đệ quy với giá trị là mảng
                $result->$key = is_array($value) ? self::arrayToObject($value) : $value;
            }
            return $result;
        } else {
            // Nếu tất cả key đều là số, chuyển đổi từng phần tử của mảng đánh chỉ mục
            return array_map([self::class, 'arrayToObject'], $array);
        }
    } else {
        // Nếu không phải là mảng, trả về giá trị gốc
        return $array;
    }
  }
