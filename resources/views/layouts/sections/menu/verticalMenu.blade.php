  ...
  <ul class="menu-inner py-1">
    @foreach ($menuData[0]->menu as $menu)
      ...
      if(isset($menu->submenu)){
          $permissionGroup = isset($menu->permission_group) ? $menu->permission_group : null;
          
          if($permissionGroup){
            $checkPermissionGroup = Permission::checkPermissionGroup($permissionGroup);
            
            if(!$checkPermissionGroup) continue;
          }
        }
      @endphp
      ...
      <li class="menu-item {{$activeClass}}">
        <a href="{{ isset($menu->url) ? url($menu->url) : 'javascript:void(0);' }}" class="{{ isset($menu->submenu) ? 'menu-link menu-toggle' : 'menu-link' }}" @if (isset($menu->target) and !empty($menu->target)) target="_blank" @endif>
          ...
          @isset($menu->badge)
            <div class="badge bg-{{ $menu->badge[0] ?? null }} rounded-pill ms-auto">{{ $menu->badge[1] ?? null }}</div>
          @endisset
        </a>
